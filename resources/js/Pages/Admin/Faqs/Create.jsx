...
export default function FaqCreate() {
    const { data, setData, post, processing, errors } = useForm({ question: '', answer: '', is_active: true });
                ...
                <Card><CardContent className="pt-6">
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <Textarea value={data.question} onChange={e => setData('question', e.target.value)} placeholder="FAQ question" rows={2} />
                            {errors.question && <p className="text-red-500 text-xs mt-1">{errors.question}</p>}
                        </div>
                        <div>
                            <Label>Answer *</Label>
                            <Textarea value={data.answer} onChange={e => setData('answer', e.target.value)} placeholder="FAQ answer" rows={5} />
                        ...
                        <div className="flex gap-3 pt-2">
                            <Button type="submit" disabled={processing} className="bg-brand-dark text-brand-green hover:bg-brand-gray">{processing ? 'Creating...' : 'Create FAQ'}</Button>
